...
int main(void)
{
    ...
    i = pthread_create( &thrid_up, NULL, (void * (*)(void *))thread_repeat, NULL);
    if (i != 0) {
...
/* -------------------------------------------------------------------------- */
/* --- THREAD: RECEIVING PACKAGE AND RESENDING THEM ---------------------- */
...
void thread_repeat(void) {
    ...
    while (!exit_sig && !quit_sig) {
        ...
        for (i=0; i < nb_pkt; ++i) {
            ...
            tx_pkt.rf_chain = 0;
            tx_pkt.rf_power = 20;
            tx_pkt.modulation = p->modulation;
            tx_pkt.bandwidth = p->bandwidth;
            tx_pkt.datarate = p->datarate;
            tx_pkt.coderate = p->coderate;
            tx_pkt.invert_pol = 0;
            tx_pkt.f_dev = 0;
            tx_pkt.preamble = 8;
            tx_pkt.no_crc = 0;
            tx_pkt.no_header = 0;
            tx_pkt.size = p->size;
            memcpy ((void *)tx_pkt.payload, (void *)p->payload, p->size);

            MSG("---------------\n");
            MSG("MESSAGE SENDED:\n");
            MSG("Freq_hz: %d\n", tx_pkt.freq_hz);
            MSG("Bandwidth: ");
            switch (tx_pkt.bandwidth) {
                case BW_125KHZ:
                    MSG("125 KHZ\n");
                    break;
                case BW_250KHZ:
            ...
            MSG("Polarity: ");
            tx_pkt.invert_pol ? MSG("negative\n"):MSG("positive\n");  
            MSG("Size: %d\n", tx_pkt.size);
            MSG("------------------------------------\n");
            MSG("------------------------------------\n");
